...
try {
  ...
  if (data.list && data.list.length > 0) {
    ...
    data.list.slice(0, 3).forEach((entry, i) => {
      ...
      console.log(`  [${i+1}] ${dt.toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' })} - ${entry.main.temp}°C, ${entry.weather[0].main}`);
    });
    ...
    console.log('  - Time:', closestDt.toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' }));
    ...
    console.log('  - Rain probability:', Math.round((closest.pop || 0) * 100) + '%');
    
    // Test seating recommendation
    const { recommendSeating } = await import('./utils/seatingRecommendation.js');
