    ...
    if (message.toLowerCase().includes('weather') && bookingData?.bookingDate) {
      ...
      if (bookingDateObj > maxDate || bookingDateObj < today) {
        return res.status(400).json({
          error: 'Weather data only available for bookings within the next 5 days',
          response: 'Sorry, I can only provide weather information for bookings within the next 5 days.',
          bookingData,
          isComplete: false,
        });
      }
      
      const { weatherInfo, seatingRecommendation, error } = await getWeatherAndRecommendation(
      ...
      if (error) {
        return res.status(400).json({
          error,
          response: 'Sorry, weather data is not available for that date. Please choose a date within the next 5 days.',
          bookingData,
          isComplete: false,
        });
      }
      ...
      return res.json({
        response: `Weather looks ${weatherInfo?.condition || 'unknown'}. Recommending ${seatingRecommendation} seating.`,
        weatherInfo,
